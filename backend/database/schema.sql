...
ALTER TABLE repositories ADD CONSTRAINT repositories_status_check CHECK (status IN ('draft', 'pending', 'approved', 'rejected', 'revision'));
UPDATE repositories SET research_date = make_date(research_year, 1, 1) WHERE research_date IS NULL AND research_year IS NOT NULL;
...
CREATE TRIGGER trg_keywords_updated_at BEFORE UPDATE ON keywords FOR EACH ROW EXECUTE FUNCTION set_updated_at();
