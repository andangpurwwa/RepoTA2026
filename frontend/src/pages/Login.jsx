...
export default function Login() {
  ...
  const subtitle = useMemo(() => {
    if (mode === 'register') return 'Lengkapi data akun mahasiswa untuk masuk ke sistem RepoTA.';
    if (mode === 'forgot') return 'Kode OTP akan dikirim ke email akun yang terdaftar.';
  ...
  async function handleLogin(e) {
    ...
    if (!identifier.trim() || !password) {
      setError(tab === 'admin' ? 'Email admin dan password wajib diisi.' : 'Email atau NIM dan password wajib diisi.');
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 via-surface to-white flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-xl">
        ...
        <section className="card p-6 sm:p-8 rounded-3xl shadow-modal">
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="label">{tab === 'admin' ? 'Email Admin' : 'Email atau NIM'}</label>
                <input
                  className="input"
                  placeholder={tab === 'admin' ? 'Masukkan email admin' : 'Masukkan email atau NIM'}
                  value={identifier}
              ...
              <div className="flex items-center justify-between gap-3 text-sm">
                ...
                <button type="button" onClick={() => switchMode('forgot')} className="font-bold text-primary hover:underline">
                  Lupa password?
                </button>
              </div>

              <button disabled={loading} className="btn-primary w-full justify-center">
                <span className={`material-symbols-rounded text-[18px] ${loading ? 'animate-spin' : ''}`}>
              ...
              {tab === 'mahasiswa' && (
                <button type="button" onClick={() => switchMode('register')} className="btn-secondary w-full justify-center">
                  Daftar Akun Mahasiswa UAD
                </button>
              )}
            </form>
          )}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Masukkan email webmail UAD"
                />
              </div>
              ...
              <button type="button" onClick={() => switchMode('login')} className="btn-secondary w-full justify-center">
                Kembali ke Login
              </button>
            </form>
          )}
          ...
          {mode === 'forgot' && (
            <form onSubmit={resetForm.sent ? resetPassword : requestOtp} className="space-y-4">
              ...
              {resetForm.sent && (
                <>
                  <div className="rounded-2xl bg-blue-50 border border-blue-200 px-4 py-3 text-sm text-blue-700">
                    Kode OTP sudah dikirim ke email akun terdaftar. Cek inbox atau spam.
                  </div>
              ...
              <button disabled={loading} className="btn-primary w-full justify-center">
                {loading ? 'Memproses...' : resetForm.sent ? 'Ubah Password' : 'Kirim OTP ke Email'}
              </button>
        ...
        <div className="mt-5 flex items-center justify-center text-sm">
          <Link to="/guest" className="text-gray-500 hover:text-primary font-semibold">
